...
function createElement(label: string, value: string | number): HTMLParagraphElement {
...
form?.addEventListener("submit", async (event: Event) => {
    ...
    if (!inputFeld.value) {
        ...
        try {
            const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${inputFeld.value.toLowerCase()}`)
            if (!response.ok) {
...
async function filterPokemonType(type: string) {
    try {
        const respone = await fetch(`${TYPE_URL}/${type}`)
